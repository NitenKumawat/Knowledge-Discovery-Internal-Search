...
async function ensureIndex(indexName) {
  ...
  await client.index(indexName).updateFilterableAttributes(['company','team','fileType','project']);
  await client.index(indexName).updateSearchableAttributes(['title','content','project']);
}

// Parse and index single file
async function parseAndIndex(filePath, meta = {}) {
  const { originalName, company, team, project } = meta;
  const indexes = ['global_documents']; // always index globally
  ...
  const doc = {
    ...
    originalPath: `/uploads/${path.basename(filePath)}` // <--- relative URL for browser
  };
...
module.exports = { parseAndIndex, getMeiliClient: () => client, getIndexName };
